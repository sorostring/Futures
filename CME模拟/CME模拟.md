...
- 比如当前S&P500=7591，那么一手MES合约的 Nominal Value = 7591X5= $ 37,955.00
- For each Micro E-mini S&P 500 contract, a move of **0.250 points**, will result in a profit or loss of **$1.25**。
...
当然，名义价值 Nominal Value就是名义价值，initial margin 占比是没有这么大的，你要开一个position，就不需要一整个 Nominal Value。
...
NinjaTrader的 Margin要求：
- Initial Margin: $2657.00
- Marintenance Margin: $2415.00
...
现在的情况就是：Sell limited (7588.75) <=Best bid(7589.75)，即 7588.75<=7589.75。能够进行成交！成交的价格并不是 7588.75，而是以7589.75成交。成交之后的盘口就会变成：
...
|你的动作| 吃掉谁 |成交价|行话|
|---|---|---|---|
|**BUY MKT**|hit the Ask|**Best Ask** Price|Buyer lifted the Ask|
|**SELL MKT**|hit the Bid|**Best Bid** Price|Seller hit the bid|

- BUY Mkt，是因为主动去买，呼应了Best Ask，所以叫做 Buyer lifted the Ask
- SELL Mkt，是因为主动去卖，呼应了 Best Bid，所以叫做 Seller hit the bid

那么在order flow 视角，我们是不是可以这样看：
...
一笔成交，永远有卖家和买家，两方的数量相等，成交价格只有一个，这是达成交易的首要条件。
但是这里，还有个关键的区别：

> 谁是主动方（Aggressor）？谁更着急一些，去“上杆子”一些？
...
**Ask Volume** = 1000
意思就是：
有1000手成交是**买方**主动发起的，都成交在Seller挂出来的价格上了。即，买方Buyer在“扫货”，这个能理解吧，buyer决心很大，有多少Ask盘他都要吃掉。
Ask Volume = 1000，不是说：挂着1000手Ask；而是说：1000手已经成交了。

总结起来就是：

|成交价|谁主动action|Order Flow解释|进一步解释|DOM中表现|
|---|---|---|---|---|
|Ask价成交|买家主动贴上去|Aggressive Buyer|Trade at Ask/Ask Volume|Ask侧亮一下。BUYER更着急进攻，突入到Ask领地|
|Bid价成交|卖家主动贴上去|Aggressive Seller|Trade at Bid/Bid volum|Bid侧亮一下。SELLER更着急进攻，突入到Bid领地|
...
Bid成交
```
专业的会员就说：**卖家**在砸盘（**lifting？hitting? the bid**）。即：卖家不停主动打买盘。
...
真实的原因是：买家愿意不断提高价格，去寻找卖家！买家越来越愿意让步。
    - 7590.00@25手，根据Price Improvement原则（下文介绍）成交价：7590.00
    - 7590.25@25手，成交价：7590.25
    - 7590.50@60手，成交价：7590.50
    - 价格就是这样被一步步打上去了。
- 交易员会说：
    ...
    - 供求关系依然在，只不过形式变了。
    - 主动Buyer不断吃Ask = 需求非常强
    - 价格上涨，更准确的说，是**买方的主动性强于卖方**，或者说，**买方愿意不断提高成交价格**。
...
这个时候应该会怎么成交呢？答案是：
> 不会出现110和90成交的订单。
> `BUY LMT 110.00` 的订单，会以`100.00`成交
> `SELL LMT 90.00` 的订单，会以`99.00`成交。
...
因为：
> Order Book 不是拍卖行。它是一个 **价格优先（Price Priority）** 的市场。
...
这么设计的目的是什么？是为了市场的稳定？我举个极端的例子。交易进行中，我随便挂一个 `SELL 1.00$ @ 1手`的订单进来，如果采用我这个价格成交的话，我就轻松扰乱了市场，一瞬间就干跌停了。我就说我不小心输错了，监管机构都不能说我扰乱市场了。或者交易进行中，我随便挂出去一个 `BUY 10000.00$ @ 1手`的BUY的单子，如果成交价就按照我这个来，那就一下子把价格干涨停了。
...
- **Price Improvement的受益者**，通常是： **Aggressive Order（Taker）**

- **成交价不是由“后来的人”决定的。**
- **成交价优先保护**订单簿中**已有订单**的报价权（Quote Priority）。而不是保护 Maker 的利益。

这也是为什么你在 CME DOM 里看到的世界，和国内教材里那个 bp-sp-cp 三数取中 的世界，看起来越来越不像同一个市场。实际上，它们背后体现的是两代交易所设计理念。


而现代 Order Book 的原则是：
- **<font color=red>后来的订单只能接受已有流动性，不能重新定义已有报价。</font>**

真正严格的说法应该是：
- **<font color=red>交易所保护的是：已存在在订单簿的，价格优先权和时间优先权。</font>**
...
- 我某种头寸，之前是几个价格都存在的，那么我平掉的时候，是平掉哪个头寸呢？这个问题比较关键，因为这可能会影响 Realized P/L的计算。你平掉的价格是同一个，那么究竟平掉哪个肯定对PnL有影响。
- 那么实际上，不同的系统可能会区别对待。

对于区分空头和多头的算法来说，一种可能的methodology是：
...
但是对于另一些系统来说，可能就会使用 **FIFO**（First In, First Out）

Then，对于CME的BUY-SELL算法来说，情况稍微有那么一点点复杂，但是还好 though。

**CME 对于 Average PX的处理是**：

- **POSITION** Table：采取 **AVERAGE PX**     来计算现有postion的成本
- 计算 **Realized P/L**，使用 **FIFO** 原则核算
- subsequently，**Unrealized P/L**也使用 **FIFO** 原则核算
...
- POSITION表格中，LONG L1手 @价格LP1
    - LP1是个加权均价
- 接下来的操作如果继续 BUY L2手@价格LP2
    - 那么新的POSITION表格就会变成：LONG (L1+L2)手@价格加权计算
    - Attention，但是，平掉的时候，CME是采用 **FIFO**
- 接下来的操作如果是 SELL，即SELL S1手@SP1
    - 如果L1=S1，那么POSITION就消失了，unrealized PL也就归零了。
    - 如果L1>S1，那么CME采用 **FIFO** 原则，剩下的LONG Position的average px不一定是LP1了。
    - 如果L1<S1，那么就是LONG都被SHORT给平了，SHORT还有富裕，最终剩余的POSITION就是：SHORT (S1-L1)@**SP1**。



## AVERAGE PX仅仅是一个参考，FIFO也重要

这是不是颠覆性的？FIFO就是 First In, First Out.
还是举例字说明吧，还是CME的MESM6合约。

**opeation-1**
order is：
BUY @3手 @ 7590.75
position is (LAST = 7592.00)
Long 3 @ BUYS 3, SELLS 0 @ AVERAGE PX 7590.75 @ Unrealized PnL +18.75 （随LAST而变） @ Realized PnL 0.00

**opeation-2**
order is：
BUY @5手 @ 7591.50
position is (LAST = 7591.00)
Long 8 @ BUYS 8, SELLS 0 @ AVERAGE PX 7591.22 @ Unrealized PnL -8.75 （随LAST而变） @ Realized PnL 0.00

**opeation-3**
order is：
SELL @6手 @ 7593.50
position is (LAST = 7594.75)
Long 2 @ BUYS 8, SELLS 6 @ AVERAGE PX 7591.50 @ Unrealized PnL +30.00 （随LAST而变）@ Realized PnL 71.25

我们完全可以看出来，虽然POSITION TABLE显示的是AVEARGE PX，但是CME还有内部的LOT帐目
...
**PROFIT/LOSS** = **REALIZED P/L** + **UNREALIZED P/L** = `33.75$` + `20$` = `53.75$`

**AVAILABLE** = **PRACTICE FUNDS** - **5294.00** = 99,316.25 - 5,294.00 = `94,022.25$`

那么，问题就来了，PRACTICE FUNDS是怎么来的呢？
这里，**PRACTICE FUNDS** = 之前值 + **UNREALIZABLE P/L** = 99,296.25 + 20.00 = `99,316.25$`，也就是说，这个concept是included **Unrealized P/L**的。（但是不需要考虑 Realzied P/L了，因为之前的99,296.25已经计入了Realized P/L的 33.75了）
...
|**UNREALIZED P/L**|还存在的头寸，由 LAST 和 position成本决定（FIFO原则）|**f**(当前头寸FIFO成本, LAST)；当前头寸情况，头寸FIFO成本，LAST成交价的函数|
...
|**PRACTICE FUND**|就是客户权益 equity|你每天的资金，不考虑出入金的前提下，每天赚钱赚钱赔钱，剩的权益。就是你打拼之后还有多少因子|
...
- 前面那张图 LAST = 7539.50；PRACTICE FUNDS = 99,316.25$
...
在T交易日的，某一个时刻t，需要控制的风险包括 **3个** 方面：

- **持有头寸的风险**：持有头寸意味着你要对外承担买或者卖的义务，因此需要一个initial margin。你只要有position，根据头寸的数量，按照交易所的要求，就要有 initial margin，这部分钱就不能动了。

- **操作中的PnL**：你不停的SELL和BUY，这就会产生 profit & loss，这个道理是不是能听懂？这部分就叫做 realized PnL，这就是你折腾来折腾去，买来卖去，所产生的 PnL。

- 随时随地情况下，如果现在有**未了结的position，也会产生 unrealized PnL**，这个unrealized PnL，相当于是 f(last price)，**LAST成交价的函数**，是随当前成交价而随时变化的。

- 手续费，commission fee：每笔交易都有手续费，这也是影响交易的一个方面。
